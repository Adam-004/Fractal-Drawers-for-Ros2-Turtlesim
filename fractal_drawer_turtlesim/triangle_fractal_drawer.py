...
def sierpinski_contour(p1, p2, p3, level):
    if level == 0:
        return [p1, p2, p3]
    ...
    # Recursive edge walking to match outer contour
    top = sierpinski_contour(p1, m1, m3, level - 1)
    right = sierpinski_contour(m1, p2, m2, level - 1)
    left = sierpinski_contour(m3, m2, p3, level - 1)

    # Join them properly, removing duplicates
    return top + right[1:] + left[1:]

class TurtleController(Node):
    def __init__(self, speed_scale=1.0):
        super().__init__('sierpinski_contour')
        self.speed_scale = speed_scale
        self.publisher = self.create_publisher(Twist, '/turtle1/cmd_vel', 10)
        self.subscriber = self.create_subscription(Pose, '/turtle1/pose', self.pose_callback, 10)
        self.pose = None
    ...
    def go_to_point(self, x, y):
        ...
        while rclpy.ok():
            ...
            if abs(angle_diff) > 0.05:
                twist.angular.z = 3.0 * angle_diff * self.speed_scale
            elif distance > tolerance:
                twist.linear.x = 2.5 * distance * self.speed_scale
            else:
                break
            self.publisher.publish(twist)
            rclpy.spin_once(self)
...
def draw_contour(turtle, points):
    total = len(points)
    for i, pt in enumerate(points + [points[0]]):  # close the loop
        hue = i / total
        r, g, b = [int(255 * c) for c in colorsys.hsv_to_rgb(hue, 0.6, 1.0)]
        turtle.set_pen(r=r, g=g, b=b, width=2)
        turtle.go_to_point(pt[0], pt[1])

def main(args=None):
    parser = argparse.ArgumentParser(description='Draw outer contour of Sierpiński triangle.')
    parser.add_argument('--level', type=int, default=2, help='Recursion depth (e.g., 1–6)')
    parsed_args = parser.parse_args(args)
    ...
    # Triangle in turtlesim window
    p1 = (2.5, 2.5)
    p2 = (8.5, 2.5)
    p3 = (5.5, 9.0)

    path = sierpinski_contour(p1, p2, p3, parsed_args.level)

    turtle.move_to_point(path[0][0], path[0][1])
    draw_contour(turtle, path)
